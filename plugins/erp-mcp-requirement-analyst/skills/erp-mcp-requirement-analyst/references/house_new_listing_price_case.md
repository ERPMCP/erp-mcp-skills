...
- 两种都展示（同时给出两套算法结果，方便对比）：默认推荐，客户能直接看到两个算法差异；
- 每套房等权均价（每套房都算 1 套）：把每套房的每平方米挂牌价相加，再除以套数；
- 按面积计算整体均价（大面积房源影响更大）：把总价合计除以总面积合计。

默认推荐哪一种必须根据业务用途说明，不能只写“算术平均/面积加权”。

如果用 WorkBuddy 原生选择卡提问，选项也必须写成：

1. `两种都展示（同时给出两套算法结果，方便对比）`
2. `每套等权（每套房都算 1 套）`
3. `面积加权（大面积房源影响更大）`

每个选项下面都要写一句普通中文解释，不要只显示选项名称。
...
- 导出当前汇总；
- 导出当前新上房源明细（来自 MCP 查询结果，不是系统导出的房源表）；
- 汇总和明细都导出。
